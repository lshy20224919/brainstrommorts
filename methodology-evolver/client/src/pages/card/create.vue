<template>
  <view class="page-create-action">
    <!-- 标题 -->
    <view class="page-header">
      <text class="page-title">新建动作</text>
      <text class="page-desc">定义一个你认为值得重复做的正确的事</text>
    </view>
...
export default {
  ...
  methods: {
    ...
    checkDraft() {
      const draft = uni.getStorageSync('draft_action')
      if (draft) {
        this.hasDraft = true
    ...
    saveDraft() {
      if (this.form.action_name || this.form.remark) {
        uni.setStorageSync('draft_action', JSON.stringify(this.form))
      }
    },
